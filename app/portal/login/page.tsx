...
// Cyber fallback while preloader script loads so the login form NEVER flashes first
const AiraLoginPreloader = dynamic(() => import("@/components/ui/AiraLoginPreloader"), {
    ssr: false,
    loading: () => (
        <div className="fixed inset-0 z-[99999] bg-slate-950 flex flex-col items-center justify-center select-none">
            <div className="w-20 h-20 rounded-3xl bg-slate-900 border border-aira-cyan/40 flex items-center justify-center glow-cyan shadow-2xl animate-pulse">
                <span className="font-orbitron font-black text-2xl text-white tracking-wider">AiRA</span>
            </div>
            <div className="w-32 h-1 rounded-full bg-white/10 mt-6 overflow-hidden">
                <div className="w-full h-full bg-gradient-to-r from-aira-cyan to-indigo-500 animate-pulse" />
            </div>
            <p className="font-orbitron text-[10px] text-slate-500 uppercase tracking-widest mt-3">Initialising Lab Portal...</p>
        </div>
    ),
});
...
export default function LoginPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-aira-bg">
            {/* ══ 1. PRELOADER ALWAYS COMES FIRST ══ */}
            ...
            {/* ══ 2. LOGIN FORM ONLY FADES IN AFTER INTRO COMPLETES ══ */}
            <AnimatePresence>
                {!showIntro && (
                    <motion.div
